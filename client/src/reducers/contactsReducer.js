...
export default function(state = initialState, action) {
	...
	switch (action.type) {
		...
		case UPDATE_CONTACT_WITH_NEW_USER_SOCKET_ID:
			...
			newState.allContacts.forEach(function(contact) {
				if (contact.matchId === action.newUserSocketInfo.userId) {
					if (
						state.selectedConversationInfo
							.selectedContactSocketId === contact.socketId
					) {
		...
		case NEW_MESSAGE:
			// TODO: optimize
			newState.allContacts.forEach(function(contact) {
				if (contact.matchId === action.senderId) {
		...
		case UPDATE_TOTAL_NUMBER_OF_UNSEEN_MESSAGES:
			newState.totalNumberOfUnseenMessages =
				action.totalNumberOfUnseenMessages;
			return newState;
		case SEEN_MESSAGES:
			newState.totalNumberOfUnseenMessages -=
				action.numberOfUnseenMessages;
			newState.allContacts.forEach(function(contact) {
		...
		case TOGGLE_BELIEF_COMPARISON:
			newState.selectedConversationInfo.showContactCard = !action.showContactCard;
			return newState;
		case UPDATE_VOTE_COMPARISON:
			newState.selectedConversationInfo.agreedAsks = action.agreedAsks;
			newState.selectedConversationInfo.disagreedAsks =
				action.disagreedAsks;
			return newState;
		case DELETE_CONVERSATION:
			for (let i = 0; i < state.allContacts.length; i++) {
				if (
					state.allContacts[i].conversationId ===
					action.conversationId
				) {
