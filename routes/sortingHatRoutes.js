...
// when the user first presses Sorting Hat this will store...
...
module.exports = app => {
  app.get("/api/sorting_hat/landing_asks", async (request, response) => {
    const nextAsks = await AskCollection.find()
      .sort({ totalVotes: -1 }) // -1 = most to least
      .limit(4);
  ...
  app.put("/api/sorting_hat/vote", requireLogin, async (request, response) => {
